...
const Text = "text-sm text-left leading-6 whitespace-nowrap px-7 py-3";
        ...
        <tr>
            {
                users ? (
                    ...
                    <td className={`${Text} float-right flex-rows gap-2`}>
                        {!data?.isAdmin && (
                        <button onClick={() => onDeleteFunction(data?._id)} className='bg-subMain text-white rounded flex-colo w-7 h-7 mt-3'>
                            <MdDelete/>
                        </button>
                        )}
                    </td>
